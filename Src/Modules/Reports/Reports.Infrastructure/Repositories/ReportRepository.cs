...
public sealed class ReportRepository(ReportsDbContext db) : IReportRepository
{
    ...
    public async Task<IReadOnlyList<Report>> GetByCustomerIdAsync(Guid customerId, CancellationToken ct)
    {
        return await db.Reports
            .Where(x => x.CustomerId == customerId)
            .ToListAsync(ct);
    }
    public async Task<IReadOnlyList<Report>> GetAllAsync(CancellationToken ct)
    {
        return await db.Reports
            .AsNoTracking()
